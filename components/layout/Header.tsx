...
function DesktopMenuItem({ item, label }: DesktopMenuItemProps) {
  ...
  if (!item.children?.length) {
    return (
      <Link href={item.href} className={triggerClassName}>
        {label}
        <span aria-hidden="true" className="absolute inset-x-4 bottom-0 h-0.5 origin-left scale-x-0 bg-primary transition-transform duration-200 group-hover:scale-x-100" />
      </Link>
    );
  }
  ...
  return (
    <DropdownMenu.Root open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DropdownMenu.Portal>
        <DropdownMenu.Content side="bottom" align="start" sideOffset={0} className="z-50 flex w-64 flex-col rounded-md bg-background py-2 shadow-lg border border-border">
...
function DesktopSubmenu({ item }: { item: NavigationItem }) {
  ...
  return (
    <div onPointerEnter={() => setIsOpen(true)} onPointerLeave={() => setIsOpen(false)}>
      <button
        type="button"
        className="group relative flex w-full items-center justify-between gap-1 rounded-md px-4 py-2 text-sm font-medium text-muted-foreground transition-colors hover:font-semibold hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-2"
        aria-expanded={isOpen}
        onClick={() => setIsOpen((open) => !open)}
      >
        {t(item.labelKey)}
        <span aria-hidden="true" className="text-xs">{isOpen ? "▴" : "▾"}</span>
        <span aria-hidden="true" className={`absolute inset-x-4 bottom-0 h-0.5 origin-left bg-primary transition-transform duration-200 ${isOpen ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"}`} />
      </button>
      {isOpen && (
...
function MobileMenuItem({ item, closeMenu, depth = 0 }: MobileMenuItemProps) {
  ...
  const mobileLinkClassName = item.children?.length
    ? `flex w-full items-center justify-between rounded-md px-3 ${depth === 0 ? "py-3 text-base" : "py-2 text-sm"} text-left font-medium text-muted-foreground hover:bg-secondary hover:font-semibold hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-2`
    : `block rounded-md px-3 ${depth === 0 ? "py-3 text-base" : "py-2 text-sm"} font-medium text-muted-foreground hover:bg-secondary hover:font-semibold hover:text-foreground focus-visible:outline-2 focus-visible:outline-offset-2`;
  ...
  return (
    <div>
      ...
      {isOpen && <div className="ml-3 border-l border-border pl-3">{item.children.map((child) => <MobileMenuItem key={child.href} item={child} closeMenu={closeMenu} depth={depth + 1} />)}</div>}
    </div>
  );
}
