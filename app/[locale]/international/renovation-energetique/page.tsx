...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return createLocalizedMetadata(locale, "/international/renovation-energetique", t("title"), t("metaDescription"));
}

export default async function EnergyRenovationPage({ params }: PageProps) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: "pages.renovation" });
  return (
